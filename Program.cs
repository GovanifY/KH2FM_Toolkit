...

{
    public static class Program
    {
        ...
        public static string ActualVersion="3.0";
        ...
        private static void KH2PatchExtractor(Stream patch, string outputname)
        {
            ...
            using (var br = new BinaryStream(patch, Encoding.ASCII, leaveOpen: true))
            {
                using (TextWriter op = new StreamWriter("output/log.log"))
                {
                    ...
                    using (TextWriter sh = new StreamWriter("output/output.sh"))
                    {
                        sh.WriteLine("mono KH2FM_Toolkit.exe -patchmaker -batch -uselog log.log -output \"{0}\"", outputname);
                    }
...
#region Loading convert patch
                    ...
                    if (Patches.patches.Count == 0 || k2e==true || convertpatch==true || extract == true) 
                    {
                        if(!(k2e==true || convertpatch==true || extract == true)) {

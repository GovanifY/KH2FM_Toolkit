...
namespace KH2FM_Toolkit
{
    public sealed class PatchManager : IDisposable
    {
        ...
        {
            byte[] v84 = new byte[] { 0x58, 0x0c, 0xdd, 0x59, 0xf7, 0x24, 0x7f, 0x4f };
            int i = -1, l = buffer.Length;
            while (l > 0) { buffer[++i] ^= v84[(--l & 7)]; }
        }

        public static void GYXor(byte[] buffer)
        {
            byte[] v84 = new byte[] { 0x47, 0x59, 0x4b, 0x35, 0x9a, 0x7f, 0x0e, 0x2a };
            int i = -1, l = buffer.Length;
            while (l > 0) { buffer[++i] ^= v84[(--l & 7)]; }
        }
        ...
        internal class Patch : IDisposable
        {
            ...
            public bool IsInKH2 { get { return this.Parent == 0; } }
            public bool IsInOVL { get { return this.Parent == 1; } }
            public bool IsinISO { get { return this.Parent == 2; } }
            ...
            public bool IsRelink { get { return this.Relink != 0; } }
            public void Dispose() { if (this.Stream != null) { this.Stream.Dispose(); this.Stream = null; } }
        }
        ...
        {
            nPatch.IsNew = true;
            if (!this.newfiles.ContainsKey(nPatch.Parent)) { this.newfiles.Add(nPatch.Parent, new List<uint>(1)); }
            if (!this.newfiles[nPatch.Parent].Contains(nPatch.Hash))
            {
                this.newfiles[nPatch.Parent].Add(nPatch.Hash);
            }
        }

        private List<Stream> patchms = new List<Stream>();

        internal PatchManager()
        {
            this.ISOChanged = this.OVLChanged = this.KH2Changed = false;
        }

        public void Dispose()
        {
            foreach (KeyValuePair<uint, Patch> patch in this.patches) { patch.Value.Dispose(); }
            this.patches.Clear();
            foreach (Stream ms in this.patchms) { ms.Dispose(); }
            this.patchms.Clear();
        }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2202:Do not dispose objects multiple times")]
        private void AddPatch(Stream ms, string patchname = "")
        {
            using (BinaryStream br = new BinaryStream(ms, System.Text.Encoding.ASCII, leaveOpen:true))
            {
                if (br.ReadUInt32() != 0x5032484b) { br.Close(); ms.Close(); throw new InvalidDataException("Invalid KH2Patch file!"); }
                this.patchms.Add(ms);
                ...
                try
                {
                    ...
                    if (num > 0)
                    {
                        ...
                        while (num > 0) { --num; Console.WriteLine(" * {0}", br.ReadCString()); }
                    }
                ...
                catch (Exception e)
                {
                    ...
                    Console.WriteLine("Error reading patch header: {0}: {1}\r\nAttempting to continue files...", e.GetType().ToString(), e.Message);
                    Console.ResetColor();
                }
                Console.WriteLine("");
                ...
                while (num > 0)
                {
                    ...
                    {
                        Console.ForegroundColor = ConsoleColor.Red;
                        Console.WriteLine("The file {0} has been included multiple times. Using the one from {1}.", HashList.HashPairs.NameFromHash(nPatch.Hash), patchname);
                        patches[nPatch.Hash].Dispose(); patches.Remove(nPatch.Hash);
                        Console.ResetColor();
                    }
                    patches.Add(nPatch.Hash, nPatch);
                    //Global checks
                    if (!this.KH2Changed && nPatch.IsInKH2 || nPatch.IsInKH2Sub) { this.KH2Changed = true; }
                    else if (!this.OVLChanged && nPatch.IsInOVL) { this.OVLChanged = true; }
                    else if (!this.ISOChanged && nPatch.IsinISO) { this.ISOChanged = true; }
                    if (nPatch.IsNew) { AddToNewFiles(nPatch); }
        ...
        public void AddPatch(string patchname)
        {
            ...
            try
            {
                ...
                {
                    fs.Position = 0;
                    this.AddPatch(fs, patchname);
                    return;
                }
                if (fs.Length > int.MaxValue) { throw new OutOfMemoryException("File too large"); }
                ...
                try
                {
                    ...
                    GYXor(buffer);
                    this.AddPatch(new MemoryStream(buffer), patchname);
                }
                ...
                finally { fs.Dispose(); fs = null; }
            }
            catch (Exception e) { if (fs != null) { fs.Dispose(); } Console.WriteLine("Failed to parse patch: {0}", e.Message); }
        }
    }
}